...
| install_compiled_genomics_deps | conda install -y -n sea-ad-scenicplus -c conda-forge -c bioconda pybedtools=0.9.1 bedtools cython numpy pandas scipy |
| install_build_backends | conda run -n sea-ad-scenicplus python -m pip install 'poetry<1.2' poetry-core |
...
PY |
| install_scenicplus | cd /tmp && rm -rf scenicplus && git clone https://github.com/aertslab/scenicplus && cd scenicplus && git checkout development && conda run -n sea-ad-scenicplus python -m pip install --no-build-isolation . |
