...
def _insert_metadata_identity(con_out: sqlite3.Connection, metadata_sqlite: Path) -> tuple[int, int, int, int]:
    con_in = sqlite3.connect(f"file:{metadata_sqlite}?mode=ro&immutable=1", uri=True)
    cur = con_in.cursor()
    totals = cur.execute(
        "select count(*),count(distinct donor_id),count(distinct operator_index),count(distinct matrix_id) "
        "from cells where partition=?", (PARTITION,)
    ).fetchone()
    seen = 0
    q = (
        "select cell_id,donor_id,matrix_id,operator_index from cells "
        "where partition=? order by rowid"
    )
    batch = []
    for row in cur.execute(q, (PARTITION,)):
        batch.append((str(row[0]), str(row[1]), str(row[2]), int(row[3])))
        if len(batch) >= 50_000:
...
def bind_full104_blocks(
    *,
    ...
    con.execute("create table authority(cell_id text primary key, donor_id text not null, matrix_id text not null, operator_index integer not null)")
    con.execute("create table materialized(cell_id text primary key, donor_id text not null, matrix_id text not null, operator_index integer not null)")
    totals = _insert_metadata_identity(con, metadata_sqlite)
    if totals != (_expected_cells, _expected_donors, _expected_operators, _expected_matrices):
        con.close()
    ...
    try:
        for i, r in enumerate(rows):
            ...
            with mp.open(newline="") as mf:
                ...
                for x in mr:
                    ...
                    if float(x["primary_row_weight"]) <= 0 or int(x["source_library"]) < 0:
                        raise RuntimeError(f"STOP_FULL104_BLOCK_META_VALUE_SEMANTICS:{r['block_key']}")
                    batch.append((str(x["canonical_cell_id"]), str(x["donor_id"]), matrix_id, op))
                if n != int(r["rows"]):
                ...
                try:
                    ...
                    raise RuntimeError(f"STOP_FULL104_DUPLICATE_CANONICAL_CELL_ID:{r['block_key']}") from e
                materialized_rows += n
            if i % 128 == 127:
                con.commit()
        con.commit()
        ...
        mismatch = con.execute(
            ...
            "where a.cell_id is null or a.donor_id!=m.donor_id or a.matrix_id!=m.matrix_id or a.operator_index!=m.operator_index"
        ).fetchone()[0]
        missing = con.execute(
