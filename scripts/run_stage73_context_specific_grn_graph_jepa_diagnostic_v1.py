...
def update_docs(cfg: dict[str, Any], lock: pd.DataFrame, pf: pd.DataFrame) -> None:
    body = (
        "Stage73 benchmarked the Stage72B Morabito/GSE174367 microglia candidate "
        "coactivity graph against no-graph, STRING, target-shuffled, and gene-label "
        "permuted graph controls using the frozen Stage71 rare-cell donor-held-out "
        "machinery. It is a diagnostic graph-prior test only: no clean external "
        "validation, causal regulatory, therapeutic, gene-ablation, or validated-GRN "
        "claim is made."
    )
    for key in ["active_status", "v3_scorecard_md"]:
        mod.update_section(cfg["inputs"][key], "Stage 73 context-specific GRN graph-JEPA diagnostic", body)
    ...
    for col in SCORECARD_COLUMNS:
        if col not in df.columns:
            df[col] = ""
    row = {
        "scorecard_item": "Stage73 context-specific GRN graph-JEPA diagnostic",
        "status": "complete",
        "stage": "Stage73",
        ...
        "current_value": f"context_graph_lock={bool(lock['context_graph_prediction_lock'].iloc[0])}; stage73_run_pass={bool(pf['stage73_run_pass'].iloc[0])}",
        "pass_fail": "pass" if bool(pf["stage73_run_pass"].iloc[0]) else "fail",
        ...
        "stage_id": "stage73_context_specific_grn_graph_jepa_diagnostic",
        ...
        "result": "see stage73_prediction_lock_decision_v1.csv",
...
def run(cfg: dict[str, Any]) -> None:
    ...
    A_shuf, reg_shuf = candidate_graph_matrix(genes, resolve(cfg["inputs"]["stage72b_candidate_edges"]), "shuffled_target_stage72b_grn", int(cfg["references"]["random_seed"]))
    A_perm, reg_perm = candidate_graph_matrix(genes, resolve(cfg["inputs"]["stage72b_candidate_edges"]), "permuted_gene_labels_stage72b_grn", int(cfg["references"]["random_seed"]))
    ...
    pf["stage73_run_pass"] = pf[["inputs_found", "stage72b_ready_input", "all_required_outputs_written", "safety_audit_pass"]].all(axis=1)
    ...
    report = f"""# Stage73 context-specific GRN graph-JEPA diagnostic
...
""",
    ...
    write_text(f"# Stage73 claim-boundary final check\n\n{md(claim)}\n", outputs["claim_boundary_final_check"])
    print(f"stage73_run_pass={bool(pf['stage73_run_pass'].iloc[0])}")
...
def main() -> None:
    ...
    parser.add_argument("--config", default="configs/agent/stage73_context_specific_grn_graph_jepa_diagnostic_v1.yaml")
