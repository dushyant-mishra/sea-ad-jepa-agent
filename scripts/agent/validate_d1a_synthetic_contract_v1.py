#!/usr/bin/env python3
"""Validate the prospective D1-A synthetic/u0-safe prototype contract."""
...
EXPECTED_TERMINAL = (
    "D1A_CONTRACT_FROZEN_PROSPECTIVELY__NO_CONFIRMATORY_PASS_FAIL__REAL_D1_UNAUTHORIZED"
)
...
def validate_contract(c: dict[str, Any]) -> dict[str, Any]:
    ...
    if c.get("schema") != "D1A_SYNTHETIC_ESTIMATION_ATLAS_CONTRACT_V1":
        failures.append("schema mismatch")
    if c.get("status") != "PROSPECTIVE_SYNTHETIC_U0_SAFE_ONLY__REAL_D1_UNAUTHORIZED":
    ...
    for name in (
        "program_table", "cell_ranking_table", "molecular_table",
        "donor_table", "source_table", "hypothesis_catalog",
    ):
    ...
    if provenance.get("algorithm_id") != "D1A_SYNTHETIC_ESTIMATION_ATLAS_V1":
    ...
    return {
        "schema": "d1a-contract-validation-v1",
        "failures": failures,
