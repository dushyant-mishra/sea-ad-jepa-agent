#!/usr/bin/env python3
"""Fail-closed V3 freeze audit for the unified teacher/student integration."""
...
SOURCE_MANIFEST = Path("docs/agent/TEACHER_STUDENT_INTEGRATED_SOURCE_MANIFEST_V3.csv")
SOURCE_ROOT = Path("docs/agent/TEACHER_STUDENT_INTEGRATED_SOURCE_ROOT_V3.txt")
FREEZE = Path("docs/agent/TEACHER_STUDENT_INTEGRATION_FREEZE_CANDIDATE_V3.json")
TEST_SELECTION = Path("docs/agent/TEACHER_STUDENT_ACTIVE_TEST_SELECTION_V3.txt")
...
EXPECTED_SOURCE_ROOT = "cd7faf6dd48f58387597b05f2e143ac629e1b74418d9720c4acdc9fcf4dfb584"
...
def audit(root: Path) -> dict[str, Any]:
    ...
    if failures:
        return {
            "schema": "TEACHER_STUDENT_INTEGRATION_FREEZE_AUDIT_V3",
            "failures": failures,
            "terminal": "STOP_TEACHER_STUDENT_INTEGRATION_FREEZE_AUDIT_V3",
        }
    ...
    if freeze.get("schema") != "TEACHER_STUDENT_INTEGRATION_FREEZE_CANDIDATE_V3":
    ...
    if freeze.get("supersedes_local_candidate") != "TEACHER_STUDENT_INTEGRATION_FREEZE_CANDIDATE_V2":
        failures.append("V2 local candidate supersession not explicit")

    authority_text = (root / SOURCE_AUTHORITY).read_text(encoding="utf-8")
    if "TEACHER_STUDENT_INTEGRATED_SOURCE_MANIFEST_V3.csv" not in authority_text:
        failures.append("executing source authority does not name V3 manifest")
    if EXPECTED_SOURCE_ROOT not in authority_text:
        failures.append("executing source authority does not pin V3 source root")
    ...
    if any("integration_freeze_v1" in path or "integration_freeze_v2" in path for path in selection):
    ...
    return {
        ...
        "failures": failures,
        "terminal": (
            "PASS_TEACHER_STUDENT_INTEGRATION_FREEZE_AUDIT_V3"
            if not failures
            else "STOP_TEACHER_STUDENT_INTEGRATION_FREEZE_AUDIT_V3"
        ),
    }
