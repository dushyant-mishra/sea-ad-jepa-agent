...
def html_doc(css,js,data):
    return '<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Stage79 Graph Control Explorer</title><link rel="icon" href="data:"><style>'+css+'</style></head><body><header><h1>Stage79 Graph Control Explorer</h1><p>Read-only Cytoscape.js and Plotly.js explorer for frozen Stage79 graph-control comparisons.</p></header><main class="app"><aside class="left"><label>Scenario</label><select id="scenario"></select><label>Regulator</label><select id="regulator"></select><label>Direction</label><select id="direction"></select><label>Magnitude</label><select id="magnitude"></select><label>Metric</label><select id="metric"></select><label>Control type</label><select id="controlType"></select><label>Control graph / seed</label><select id="controlGraph"></select><label>Network selector</label><select id="graphMode"><option value="real">real graph</option><option value="control">selected control graph</option></select><label>Edge view</label><select id="edgeView"><option value="active">active regulator only</option><option value="all">all control edges</option></select><label>Donor display</label><select id="seedMode"><option value="mean">null mean across seeds</option><option value="selected">selected seed</option></select><div class="panel tiny">Edges are labeled as Coactivity-signed candidate influence for the frozen real graph and Control-only propagated edge for controls. Positive donor differences mean numerically larger in the real graph, not better.</div></aside><section class="graph-wrap"><div class="toolbar"><button id="fit">Fit</button><button id="reset">Reset</button><button id="clear">Clear selection</button><span id="summary" class="scenario-line"></span></div><div id="cy"></div></section><aside class="right"><div class="panel"><h2>Real versus null distribution</h2><div id="distPlot" class="plot"></div></div><div class="panel"><h2>Effect-size summary</h2><div id="effectPlot" class="plot small"></div><pre id="effectText"></pre></div><div class="panel"><h2>Donor paired differences</h2><div id="donorPlot" class="plot"></div></div><div class="panel"><h2>Control diagnostics</h2><pre id="diagnostics"></pre></div><div class="panel"><h2>Inspector</h2><pre id="inspector"></pre></div></aside></main><script>window.__STAGE79_GRAPH_CONTROL_EXPLORER__='+json.dumps(data,separators=(',',':'),sort_keys=True,allow_nan=False)+'</script><script>'+js+'</script></body></html>'
...
def main():
    ...
    print(json.dumps({'stage79_visualization_pass':True,'graphs':len(graphs),'scenarios':len(distributions['scenarios']),'metrics':len(distributions['metrics']),'runtime_network_request_count':0},indent=2,sort_keys=True)); return 0
