...
SCHEMA='JEPA_T0_TECHNICAL_SENSITIVITY_REGISTRY_V2'
MEMBER='T0_TECHNICAL_SENSITIVITY_REGISTRY.json'
DIRECT=['Q_DEPTH','Q_DETECT']
ADDITIONAL_SEMANTICS='CUMULATIVE_ON_TOP_OF_Q_DEPTH_Q_DETECT'
...
def build_technical_registry(outdir,blocks:list[dict],source_authority_hashes:dict[str,str],audit_note:str):
    ...
    for b in blocks:
        ...
        if any(c in DIRECT for c in cols): raise ValueError('additional block must not repeat frozen Q_DEPTH/Q_DETECT columns')
    ...
    obj={'schema':SCHEMA,'audit_complete':True,'mandatory_direct_observation_block':DIRECT,'additional_block_semantics':ADDITIONAL_SEMANTICS,'additional_blocks':norm,'source_authority_hashes':dict(sorted(source_authority_hashes.items())),'audit_note':audit_note}
    out=Path(outdir)
...
def load_technical_registry(outdir):
    ...
    keys={'schema','audit_complete','mandatory_direct_observation_block','additional_block_semantics','additional_blocks','source_authority_hashes','audit_note'}
    if set(obj)!=keys or obj.get('schema')!=SCHEMA or obj.get('audit_complete') is not True or obj.get('mandatory_direct_observation_block')!=DIRECT or obj.get('additional_block_semantics')!=ADDITIONAL_SEMANTICS or not isinstance(obj.get('audit_note'),str) or not obj['audit_note']:
        raise ValueError('technical registry metadata mismatch')
    names=set(); blocks=obj.get('additional_blocks')
    if not isinstance(blocks,list): raise ValueError('technical registry blocks invalid')
    for b in blocks:
        if not isinstance(b,dict) or set(b)!={'name','columns'} or not isinstance(b['name'],str) or not b['name'] or b['name'] in names or not isinstance(b['columns'],list) or not b['columns'] or any(not isinstance(c,str) or not c for c in b['columns']) or len(set(b['columns']))!=len(b['columns']) or any(c in DIRECT for c in b['columns']): raise ValueError('invalid stored technical block')
        names.add(b['name'])
