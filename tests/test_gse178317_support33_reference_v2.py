...
def fixture():
    ...
    w.writerows([
        ...
        ["C",18,1,-3.0,"",-0.2,1.]])
    support={}
    for target,good in [("A",True),("B",True),("C",False)]:
    ...
    receipt={
        ...
        "verdict_basis":{
            ...
            "min_target_and_ntc_cells_per_lane":10,"usable_targets":2,
...
def test_claimed_supported_total_must_match():
    text,receipt=fixture()
    receipt["verdict_basis"]["usable_targets"]=3
    with pytest.raises(Stop):
        run(text,receipt)
