...
def qualify(r=None, a=None, reg=None):
    reg = reg or registry()
    a = a or authority(reg)
    r = r or receipt(a, reg)
    return qualify_production_geometry_gpu_receipt(
        r,
        authority=a,
        protected_registry_authority=reg,
    )
...
def test_historical_128x8_receipt_cannot_masquerade_as_production():
    ...
    with pytest.raises(RuntimeError, match="HISTORICAL_128X8_RECEIPT_IS_NOT_PRODUCTION_GEOMETRY"):
        qualify_production_geometry_gpu_receipt(
            ...
            authority=authority(reg),
            protected_registry_authority=reg,
        )


def test_dimension_artifact_substitution_stops():
    reg = registry(); a = authority(reg); r = receipt(a, reg)
    r["bindings"]["production_dimension_artifact_sha256"] = "9" * 64
...
def test_packing_artifact_substitution_stops():
    reg = registry(); a = authority(reg); r = receipt(a, reg)
    r["bindings"]["packing_restart_invariance_artifact_sha256"] = "9" * 64
    with pytest.raises(RuntimeError, match="BINDING_SUBSTITUTION"):
        qualify(r, a, reg)
...
def test_historical_receipt_substitution_stops():
    reg = registry(); a = authority(reg); r = receipt(a, reg)
    r["bindings"]["historical_c2_gpu_receipt_sha256"] = "9" * 64
    with pytest.raises(RuntimeError, match="BINDING_SUBSTITUTION"):
        qualify(r, a, reg)
...
def test_synthetic_loader_cannot_satisfy_production_geometry():
    reg = registry(); a = authority(reg); r = receipt(a, reg)
    r["real_full104_reader_batch_used"] = False; r["synthetic_loader_used"] = True
...
def test_geometry_must_be_data_derived():
    reg = registry(); a = authority(reg); r = receipt(a, reg)
    r["geometry"]["source"] = "HISTORICAL_128X8"
...
def test_dead_protected_gradient_stops():
    reg = registry(); a = authority(reg); r = receipt(a, reg)
    r["protected_registry"]["gradients_live"] = reg.expected_tensors - 1
...
def test_geometry_must_be_frozen_before_execution():
    reg = registry()
    good = authority(reg)
    bad = authority(reg, production_geometry_frozen_before_gpu_run=False)
...
def test_registry_sha_must_match_gpu_authority():
    ...
    with pytest.raises(RuntimeError, match="PROTECTED_REGISTRY_AUTHORITY_MISMATCH"):
        qualify_production_geometry_gpu_receipt(
            receipt(authority(reg), reg),
            authority=authority(reg),
            protected_registry_authority=other,
        )
