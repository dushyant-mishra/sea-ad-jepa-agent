...
def test_overlay_rejects_legacy_v3_review_terminal_and_unbound_package() -> None:
    attacked = _valid_overlay()
    attacked["independent_review"]["terminal"] = (
        "PASS_TEACHER_STUDENT_UNIFIED_V3_INDEPENDENT_REVIEW__TRAINING_STILL_UNAUTHORIZED"
    )
    result = validate_overlay(attacked)
    assert result["terminal"].startswith("STOP_")
    assert any("exact V4 independent-review PASS" in item for item in result["failures"])

    attacked = _valid_overlay()
    attacked["independent_review"].pop("reviewed_package_root")
    result = validate_overlay(attacked)
    assert result["terminal"].startswith("STOP_")
    assert any("reviewed package root" in item for item in result["failures"])


def test_overlay_binds_external_review_to_exact_source_root() -> None:
    attacked = _valid_overlay()
    attacked["independent_review"]["reviewed_source_manifest_root"] = "9" * 64
    result = validate_overlay(attacked)
    assert result["terminal"].startswith("STOP_")
    assert any("source root" in item for item in result["failures"])
