...
def pcap(count,ok=True):
    return ControlCapacityCalibrationReceiptV1("TARGET_PANEL_SIZE_CAPACITY_CALIBRATION_V1",count,h(f"p-{count}"),h(f"s-{count}"),h("calibration-precision"),0.03 if ok else 0.0,0.01 if ok else -0.001,104,count,4096,95,100,True)
...
def ncap(cap,ok=True):
    return ControlCapacityCalibrationReceiptV1("NONLINEAR_CAP_CAPACITY_CALIBRATION_V1",cap,h(f"np-{cap}"),h(f"ns-{cap}"),h("precision"),0.03 if ok else 0.0,0.01 if ok else -0.001,104,128,4096,95,100,True)
