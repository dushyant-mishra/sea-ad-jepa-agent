...
def make_authority() -> TeacherTargetSemanticsAuthorityV1:
    return TeacherTargetSemanticsAuthorityV1(
        ...
        teacher_input_support_authority_sha256="2" * 64,
        ...
        target_address_query_authority_sha256="3" * 64,
        student_visible_support_authority_sha256="4" * 64,
        scientific_weight_authority_sha256="5" * 64,
        masking_authority_sha256="6" * 64,
        gradient_boundary_authority_id="EXPLICIT_GRADIENT_BOUNDARY",
        ema_boundary_authority_sha256="7" * 64,
    )


def test_all_semantic_choices_are_explicit() -> None:
    sig = inspect.signature(TeacherTargetSemanticsAuthorityV1)
...
def test_semantics_authority_is_hash_bound_and_training_off() -> None:
...
def test_bad_dependency_root_fails_closed() -> None:
    authority = dataclasses.replace(make_authority(), masking_authority_sha256="bad")
    with pytest.raises(ValueError, match="masking_authority_sha256"):
        authority.validate()


def test_source_does_not_select_inherited_v4_target_semantics() -> None:
