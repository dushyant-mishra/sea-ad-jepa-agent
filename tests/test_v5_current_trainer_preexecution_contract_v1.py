...
def roots(): return {name: str(i%10)*64 for i,name in enumerate(CURRENT_V5_UPSTREAM_AUTHORITY_ROOTS,1)}
def make_authority(): return CurrentTrainerPreexecutionAuthorityV1(roots(),'a'*64,'b'*64,False,False)
def test_root_vocab_exact_and_receipt_adds_preexecution():
    assert CURRENT_V5_UPSTREAM_AUTHORITY_ROOTS == (
        'full104_substrate_sha256','representation_authority_sha256','support_estimability_authority_sha256','base_training_estimand_sha256',
