...
class VerifierF1RepairTests(unittest.TestCase):
    ...
    def test_verifier_j_actual_git_source_authority_is_stage_bound(self):
        ...
        implementation = "a3c4452d48cedc8650ba9de4a9d6737cc926544c"
        root_commit = "2f33a8a1662e98de03cbb28604d8bfbb2d9477bb"
        self.assertIn(head, {implementation, root_commit})
        ...
        if head == root_commit:
            parent = finalizer.git_output("rev-parse", "HEAD^")
            changed = set(filter(None, finalizer.git_output(
                "diff", "--name-only", f"{implementation}..{root_commit}"
            ).splitlines()))
            environment = json.loads((
                WORKTREE / "outputs/contextual_teacher_target_v1_f1_real_reader_forward_executor_preflight_20260903/F1_PREFLIGHT_WSL_ENVIRONMENT_AUTHENTICATION.json"
            ).read_text(encoding="utf-8"))
            self.assertTrue(finalizer.validate_commit_chain(
                actual_head=head,
                benchmark_commit=environment["benchmark_execution_commit"],
