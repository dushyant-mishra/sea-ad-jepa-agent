"""External B2 authority-chain red attacks.

These reproduce parent-splice / stored-root / unbound-operational-path defects
against producing head 760c68efdba022a14670fa5bcaa4c985e28b686d.

They intentionally avoid prescribing the future NPZ parser API.
"""
...
def test_logical_builder_refuses_membership_different_from_closure_parent():
...
def test_logical_authority_must_bind_its_parent_closure_root():
    _mem, _manifest, _closure, logical = _world()
    expected = logical["logical_row_authority_root_sha256"]
    forged = dict(logical)
...
def test_execution_used_paths_cannot_mutate_without_verifier_failure(field, value):
    _mem, _manifest, _closure, logical = _world()
    expected = logical["logical_row_authority_root_sha256"]
    forged = dict(logical)
