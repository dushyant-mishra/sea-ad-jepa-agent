...
- Claude's 58 MB data commit `52f20a7a` is an ancestor of that head (confirmed ancestor; do not use an unqualified commit-distance figure). Physical output binaries, receipts and producers are already on GitHub under `analysis/therapeutic_perturbation_etl/`. Do not re-upload duplicate binaries or merge development results into main merely to make them accessible.
...
Compared with the depositors' CRISPRbrain analysis of the SAME experiment: 35/35 historically comparable targets agree in direction, Spearman ~0.720 **in the original, not support-filtered comparison**; AARS and LSM6 fail the reviewed matched-well support gate, so a separate 33-target comparison and freshly computed agreement/correlation are required; 18 reference-nonsignificant targets show median magnitude ratio 2.37 versus 3.07 overall. This is **pipeline/reference agreement, NOT independent replication or proof of false-positive control**. The 18 null-labeled targets are reference FDR>=0.05, not known biological nulls. The V2 z=5/minimum support thresholds were chosen after an inspected bounded smoke run: **DEVELOPMENT ONLY**.
...
1. Re-fetch live PR #77 and read its `outputs/README.md`, producer scripts, physical receipts and **nine CI workflow results on `67981c8ef6e158d865470e70bca81cbe289be3dd`**.
...
3. The corrected V2 receipt is already committed. **Audit every downstream script, receipt consumer and report** to ensure none promotes superseded V1 `PASS` to independent validation. Compute a separately labeled **33-target matched-well support-qualified** comparison after excluding AARS and LSM6; recompute sign agreement, correlations and effect-size ratios rather than inheriting the historical 35-target values. Do not assume the 3.07x magnitude ratio is caused by stricter guide calling; test that mechanism with a caller-sensitivity analysis.
4. Publish or identify a machine-readable full-SHA-256 manifest for **every committed output binary**, with exact bytes, source commit and a verification command/CI; short prefixes are not standalone integrity checks. Freeze/physically authenticate the HGNC/Ensembl mapping source files, build and test the actual crosswalk, apply to all eight studies, report missing/collision/assay masks.
