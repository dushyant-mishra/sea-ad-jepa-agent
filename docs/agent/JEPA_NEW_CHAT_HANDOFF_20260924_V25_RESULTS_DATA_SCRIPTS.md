...
IMPORTANT RECEIPT CAVEAT: the original `evidence/gse178317_recovery/gse178317_crisprbrain_validation_receipt_v1.json` still contains a historical, **superseded** phrase that the two analyses 'share no intermediate' and a generic PASS. That is false as a statement of independent validation: both share original reads. The corrected `GSE178317_RECOVERY_QUALIFIED_20260924.md` and `outputs/README.md` control interpretation. Preserve the original receipt as history or publish a versioned corrected successor; never propagate its stale independence claim.
