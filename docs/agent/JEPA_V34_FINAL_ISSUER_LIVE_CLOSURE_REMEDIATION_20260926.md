...
- **Additional distinct trust-boundary gap confirmed by self-audit:** `CurrentTrainingAuthorityV1` is a publicly constructible frozen dataclass; `qualified_optimizer_guard_v3.py` checks its self-computed hash and receipt roots, not that the token actually originated from `issue_training_authority_v1`. Our corrected older optimizer tests now construct explicitly labeled `SYNTHETIC_GUARD_FIXTURE_ONLY` tokens and demonstrate this distinction. The issuer repair alone does NOT close end-to-end authorization. Before B2, design and test a durable, source-authenticated issuer-to-guard provenance mechanism (including restart/replay behavior), and reject direct fabricated authority tokens at the production guard boundary. No synthetic optimizer fixture may be cited as a production issuance positive control.
