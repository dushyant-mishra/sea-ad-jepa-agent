...
`reader_fit_development_sampler_v1.py` **first executes the PR132 physical frozen-pass1/metadata bridge** (not a caller-supplied mock receipt), then rehashes and rereads the frozen pass1 through a single descriptor and revalidates the archived fit count map. The proposal draws each donor slot uniformly across 104; for each donor, draws distinct selection rows without replacement within a single update, keeping every presentation's **unconditional marginal** probability `q_i = 1/(104*n_d) = p_i`. Thus marginal importance `p_i/q_i = 1`. The joint slots are dependent, unlike IID replacement sampling; do not relabel the proposal IID. Before any RNG draw, require the *entire* prospectively chosen update size to be no greater than the smallest frozen donor's eligible count; otherwise conditional acceptance of capacity-compatible donor sequences would bias the declared q=p marginal. Independent physical Aug24 metadata inspection on September 25 measured **minimum 81 cells per donor** (104 donors, 4,553,407 cells). For this precise proposal, **presentations per update must therefore be explicitly selected at or below 81**; this is a source-derived bound, NOT an inherited 128×8 or chosen production batch size. If any actual sampled donor still exceeds capacity, **STOP** as an invariant violation. No retry, no reweighting and no silently changed proposal. The existing V5 reference runner rejects duplicate stable keys, so this is a functional integration boundary, not a tuning choice.
