...
Underlying source/data lineage anchor:
...
If source/data bytes changed from the underlying 86692cde... lineage, classify the delta as CHANGED_INPUT_REQUIRES_REQUALIFICATION. If only tests/workflow changed from f62dc425..., perform guard/CI re-audit before proceeding. Do not reset, clean, stash, rewrite history, or overwrite user changes.
...
Do not use any earlier 20260918 FULL104 calibration handoff branch as current authority. The takeover-ready branch named in START_HERE/pointer is current.

## C. Verify exact scientific-anchor CI before doing new work

Scientific source/test anchor f62dc42582c8b8933955505f5127bebfaa687304 is already audited PASS/no-skips in GitHub Actions run 35373584821 for the V5 FULL104 masking runner. The immediately preceding run 35373486676 failed because the anti-spillover test still referenced nonlinear sampling authority V1; f62dc425... changed only those stale V1->V2 test references and the successor run passed.

Re-fetch .github/workflows/v5-full104-masking-runner.yml and compare the live source/test/workflow bytes. If any changed after f62dc425..., rerun exactly the focused test list and fail-on-skips gate. Any skip or new failure is a STOP. Docs-only governance/handoff changes above f62dc425... do not by themselves invalidate that scientific-byte CI.
