...
def load_assignment_authority(path,expected_sha):
 path=Path(path)
...
def aggregate(records, assignment_csv_path=None, assignment_file_sha256=None, *, test_only=False, forward_authority_sha256=None):
    ...
    assignment_authority=load_assignment_authority(assignment_csv_path,assignment_file_sha256)
    donors=sorted({str(r["donor"]) for r in records}); cells=sorted({str(r["cell"]) for r in records})
    cell_meta={}
    for r in records:
    ...
    expected=len(cells)*len(PROGRAMS)*len(EVIDENCE)*2
    if len(by)!=expected:raise ValueError("missing assignment outcome")
    forward_seen={}
    for r in records:
    ...
    if any(abs(sum(a[c] for c,(dd,_) in cell_meta.items() if dd==d)-1)>1e-14 for d in donors):raise ValueError("donor weight mass")
    result={"donors":donors,"program":{},"program_qid_margin":{},"overall":{},"query_variance":{}}
    for e in EVIDENCE:
...
def synthetic():
    ...
    out={"hierarchy_exact":abs(d1-expected_d1)<1e-14,"population_overall_vq_exact":abs(agg["query_variance"]["0.6"]["population_overall"]-v_expected)<1e-14,"negative_qid_program_veto":not attacked["gates"]["no_qid_negative_program_holm"],"assignment_query_swap_rejected":swap_rejected,"inconsistent_duplicate_reuse_rejected":reuse_rejected,"caller_variance_rejected":variance_rejected,"forward_identity_mutation_rejected":forward_rejected,"assignment_authority_mutation_rejected":authority_mutation_rejected,"donor_operator_relabel_rejected":donor_operator_relabel_rejected,"production_unfrozen_forward_rejected":production_unfrozen_forward_rejected,"base":decision,"attack":attacked};td.cleanup();return out
