...
python analysis/therapeutic_perturbation_etl/scripts/gse301119_matched_nt_null_v1.py `
  ...
  --identity-cert "D:/jepa_perturb_outputs_20260923/gse301119_neutral_identity_cert_v2/NEUTRAL_EXPORT_IDENTITY_CERTIFICATION_V2.json" `
  --draws 64 --max-cell-ratio 1.5
```

The neutral directory is an **illustrative path only**: point `--neutral-dir` to the *actual* location containing `NEUTRAL_EXPORT_RECEIPT_V1.json`. The mandatory `--identity-cert` points to the separately generated V2 certificate, which compares all four neutral export files byte-for-byte with reconstruction from the authenticated original RDS. The Python audit rechecks the certificate's digest and every certified input hash. This establishes export identity within the certifier's scope, **not** an independently validated biological estimand. Do not regenerate the 4.45 GB assets or re-stream GSE178317.
