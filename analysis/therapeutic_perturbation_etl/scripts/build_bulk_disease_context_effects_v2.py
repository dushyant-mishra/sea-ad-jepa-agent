...
from gse240609_public_geo_sample_authority_v1 import (
    EXPERIMENTAL_MATERIAL,
    validate_sample_inventory,
)

# PR77 historical physical source roots; checked again BEFORE output creation.
GSE241858_SOURCE_ROOTS = {
...
def read_table(path: Path) -> tuple[list[str], list[str], list[str], np.ndarray]:
    ...
    if not np.all(np.isfinite(X)) or np.any(X < 0):
        raise SystemExit(f"{path.name}: negative or nonfinite source count")
    if len(set(genes)) != len(genes):
        raise SystemExit(f"{path.name}: duplicated original gene identifier")
...
def main() -> int:
    ...
    if out.exists() and any(out.iterdir()):
        raise SystemExit("STOP_BULK_DISEASE_V2_OUTPUT_EXISTS__NEW_VERSIONED_DIR_REQUIRED")
    for arm, p in (("baseline", args.gse241858_baseline),
                   ("cytokine", args.gse241858_cytokine)):
        if not p.is_file() or sha256_file(p) != GSE241858_SOURCE_ROOTS[arm]:
            raise SystemExit(f"STOP_GSE241858_{arm}_SOURCE_ROOT_MISMATCH")
    files = sorted(args.gse240609_dir.glob("*gene_counts.txt.gz"))
    # Independent GEO accession-title mapping + producer raw-source SHA checks
    # before opening ANY molecular source or publishing any result.
    inventory609 = validate_sample_inventory(files)
    approved609 = {item.filename: item for item in inventory609}
    out.mkdir(parents=True, exist_ok=True)
    summary: dict = {"schema": "BULK_DISEASE_CONTEXT_EFFECTS_V2"}
    ...
    # ================= GSE240609 =================
    # The four filenames, byte digests and GEO sample titles were frozen in
    # the preflight above; no new filenames can enter this stage.
    ...
    for p in files:
        ...
        if any(not np.isfinite(x) or x < 0 for x in val):
            raise SystemExit(f"{p.name}: invalid negative or nonfinite source count")
        if len(gene) != len(set(gene)):
            raise SystemExit(f"{p.name}: duplicate source gene identifier")
        X609.append(val)
        reviewed = approved609[p.name]
        meta609.append({
            "study": "GSE240609", **reviewed.public_info(),
        })
    ...
    for neuron in sorted({m["neuron_genotype"] for m in meta609}):
        ...
        for k in np.flatnonzero(det609):
            rows609.append({
                ...
                "note": ("one sample per design cell: descriptive difference only, "
                         "biological uncertainty not estimable. RNA derives from "
                         "CD11b-purified microglia isolated after neuron coculture; "
                         "not an intrinsic cell-autonomous effect."),
            })
    if rows609:
    ...
    summary["GSE240609"] = {
        ...
        "uncertainty_estimable": False,
        "interpretation_limit": ("CD11b-purified microglia recovered AFTER neuron-"
                                 "microglia coculture. Response may depend on neuron context;"
                                 " one sample per genotype cross forbids biological SE."),
        "sample_identity_authority": "NCBI_GEO_GSE240609_PUBLIC_SAMPLE_TITLES",
        "source_file_byte_roots_checked": True,
