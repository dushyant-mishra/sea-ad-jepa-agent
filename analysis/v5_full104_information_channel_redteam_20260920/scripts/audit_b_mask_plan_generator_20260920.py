...
def plan_masks_for_target(
    *,
    ...
    """Return, per policy, the exact mask that policy would apply.

    Calls only the frozen selection/assembly functions, never the scorer.
    """
    ...
    for method in _METHODS:
        ...
        plans[method] = {
            ...
            # The addresses this policy ADDS relative to the common-random base,
            # and the ones it DISPLACES. Every burden difference between a
            # targeted policy and UNIFORM_RANDOM is carried by exactly these two
            # sets: the rest of the mask is identical by construction.
