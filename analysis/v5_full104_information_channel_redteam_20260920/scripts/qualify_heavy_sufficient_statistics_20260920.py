...
SCHEMA = "V5_FULL104_HEAVY_SUFFICIENT_STATISTICS_QUALIFICATION_V1"
...
def _chunk(rows: list[dict]) -> dict:
    """Recompute metadata aggregates for a chunk of blocks, exactly."""
...
def main() -> int:
    ...
    # cross-check per-donor library sums against the artifact's own cell vector,
    # using pass1's authenticated cell->donor map (a third, independent source).
    ...
    if cell_donor.size != expected_cells:
        ...
        for row, d in zip(donor_rows, sample):
            art_sum = int(art_libraries[cell_donor == d].sum(dtype=np.int64))
            row["source_library_sum_artifact"] = art_sum
            if art_sum != row["source_library_sum_metadata"]:
                failures.append(
                    f"donor {duniq[d]}: library sum metadata "
                    f"{row['source_library_sum_metadata']} != artifact {art_sum}")
