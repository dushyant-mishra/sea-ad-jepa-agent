...
univ=np.load(B+'/universe_6000_local.npy')
...
def std_features(rows,cols):
    ...
    for d in np.unique(dd):
        ix=dd==d; mu=A[ix].mean(0); sd=A[ix].std(0)
        A[ix]=(A[ix]-mu)/np.where(sd>1e-8,sd,1)
    return A,dd

def fit_ridge_w(ti,trd,tcol,feats,alpha=.01):
    rows=np.flatnonzero(np.isin(dcode,trd)); A,dd=std_features(rows,feats)
    y=X[rows,tcol].toarray().ravel().astype(float)
    for d in np.unique(dd): y[dd==d]-=y[dd==d].mean()
    ys=y.std()
    if ys<1e-8:return np.zeros(len(feats))
    y/=ys; G=A.T@A+alpha*len(rows)*np.eye(len(feats))
    return np.linalg.solve(G,A.T@y)

def ridge8(ti,trd,tcol):
    s=ts(ti,trd,univ).copy(); s[univ==tcol]=-1
    cand=univ[np.lexsort((univ,-s))[:64]]
    w=fit_ridge_w(ti,trd,tcol,cand)
...
def make_mask(tcol,fi,ti,tg):
    burden=int(round(.15*len(univ))); pool=univ[univ!=tcol]
    seed=int.from_bytes(hashlib.sha256(f'JEPA_SCALE_MASK|{len(univ)}|{fi}|{ti}'.encode()).digest()[:8],'big')
    base=list(map(int,np.random.default_rng(seed).choice(pool,burden-1,False)))
    M=set(base+[int(tcol)]); add=[a for a in tg if a not in M]
...
def nonlinear_score(ti,trd,vad,tcol,mask,maxf=32):
    ...
    s=ts(ti,trd,vis); feats=vis[np.lexsort((vis,-s))[:maxf]]
    ...
    model=HistGradientBoostingRegressor(max_iter=50,max_leaf_nodes=15,l2_regularization=1.0,random_state=20260917)
    model.fit(Atr,ytr); pred=model.predict(Ava)
    per=[]; ps=[]
    for d in np.unique(dva):
        ix=dva==d; yy=yva[ix]-yva[ix].mean(); pp=pred[ix]-pred[ix].mean()
        den=np.sqrt((yy@yy)*(pp@pp)); r=0.0 if den<1e-12 else float((yy@pp)/den)
        per.append(r*r); ps.append(sources[d])
    per=np.array(per); ps=np.array(ps)
    return float(np.mean([per[ps==s].mean() for s in np.unique(ps)]))

fi=int(sys.argv[1]); va=np.flatnonzero(fold==fi); tr=np.flatnonzero(fold!=fi)
rows=[]; t0=time.time()
for ti,tcol in enumerate(targets):
    base=make_mask(tcol,fi,ti,[]); u=nonlinear_score(ti,tr,va,tcol,base)
    for name,tg in [('TOP8',top8(ti,tr,tcol)),('RIDGE8',ridge8(ti,tr,tcol))]:
        sc=nonlinear_score(ti,tr,va,tcol,make_mask(tcol,fi,ti,tg))
        rows.append(dict(fold=fi,target=ti,method=name,U=u,score=sc,delta=u-sc))
    print('fold',fi,'target',ti,'elapsed',round(time.time()-t0,1),flush=True)
pd.DataFrame(rows).to_csv(f'/mnt/data/outer5200_nonlinear32_fold{fi}.csv',index=False)
