...
X=sp.load_npz(BASE+'/X_common6000.npz').tocsr(); C=np.load(BASE+'/donor_target_abs_corr.npy',mmap_mode='r')
univ=np.load(BASE+'/universe_6000_local.npy'); targets=np.load(BASE+'/scale_targets_cols.npy')[:8]
...
out=pd.DataFrame(rows); out.to_csv(f'/mnt/data/ridge8_nonlinear_fold{fi}.csv',index=False)
