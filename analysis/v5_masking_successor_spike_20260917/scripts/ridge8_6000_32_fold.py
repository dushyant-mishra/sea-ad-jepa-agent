...
BASE='/mnt/data/jepa_spike_work';X=sp.load_npz(BASE+'/X_common6000.npz').tocsr();C=np.load(BASE+'/donor_target_abs_corr.npy',mmap_mode='r');nu=6000;univ=np.load(BASE+'/universe_6000_local.npy');targets=np.load(BASE+'/scale_targets_cols.npy')[:32]
...
print('fold',fi,'elapsed',round(time.time()-t0,1));d=pd.DataFrame(rows);d.to_csv(BASE+f'/ridge8_6000_32_{fi}.csv',index=False);print(d.groupby('method').agg(mean=('delta','mean'),median=('delta','median'),win=('delta',lambda x:float((x>0).mean()))).to_string())
