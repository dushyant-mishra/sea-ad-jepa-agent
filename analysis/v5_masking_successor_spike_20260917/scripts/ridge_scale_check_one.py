...
U={n:np.load(f'{BASE}/universe_{n}_local.npy') for n in [800,2000,6000]}; addr=np.load(BASE+'/selected6000_sorted.npy'); targets=np.load(BASE+'/scale_targets_cols.npy')[:8]
...
rows=[]; t0=time.time()
import sys
only=int(sys.argv[1]) if len(sys.argv)>1 else 800
for nu in [only]:
