...
BASE='/mnt/data/jepa_spike_work';X=sp.load_npz(BASE+'/X_common6000.npz').tocsr();C=np.load(BASE+'/donor_target_abs_corr.npy',mmap_mode='r');nu=int(sys.argv[1]);univ=np.load(BASE+f'/universe_{nu}_local.npy');targets=np.load(BASE+'/scale_targets_cols.npy')[:8]
...
fi=int(sys.argv[2]);va=np.flatnonzero(fold==fi);tr=np.flatnonzero(fold!=fi);rows=[];t0=time.time()
...
print('fold',fi,'elapsed',round(time.time()-t0,1));d=pd.DataFrame(rows);d.to_csv(BASE+f'/ridge8_{nu}_{fi}.csv',index=False);print(d.groupby('method').agg(mean=('delta','mean'),median=('delta','median'),win=('delta',lambda x:float((x>0).mean()))).to_string())
