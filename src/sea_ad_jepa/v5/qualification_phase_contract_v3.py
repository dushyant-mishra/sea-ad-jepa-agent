"""Active V5 postqualification bundle requiring executable power evidence.

V3 supersedes the V2 postqualification bundle for any future production-
eligibility claim.  The older V2 bundle could accept a dependency closure whose
power parent was the report-only V3 calibration.  V3 requires dependency
closure V2, whose power parent must be an executable V4 receipt with raw-output
bindings and independent recomputation.

This module establishes eligibility only; it never authorizes training.
"""
...
POST_QUALIFICATION_EVIDENCE_V3 = POST_QUALIFICATION_EVIDENCE + (DEPENDENCY_EVIDENCE_ID,)
...
def _validate_dependency_row_v2(
    ...
    *,
    ...
    if not isinstance(report, Mapping) or report.get("schema") != "JEPA_V5_POSTQUALIFICATION_DEPENDENCY_CLOSURE_V2":
        raise ValueError("postqualification dependency closure V2 is required")
    ...
    if report.get("qc_parent_child_bound") is not True:
        raise RuntimeError("STOP_V5_DEPENDENCY_QC_NOT_BOUND")
    if report.get("executable_power_parent_child_bound") is not True:
        raise RuntimeError("STOP_V5_EXECUTABLE_POWER_NOT_BOUND")
    if report.get("power_execution_mode") != "ACTUAL_FROZEN_GATE_EXECUTION":
        raise RuntimeError("STOP_V5_ACTUAL_FROZEN_GATE_EXECUTION_NOT_BOUND")
    if report.get("independent_power_recomputation") is not True:
        raise RuntimeError("STOP_V5_POWER_RECOMPUTATION_NOT_INDEPENDENT")
...
def build_postqualification_bundle_v3(
    ...
    *,
    ...
    if not isinstance(evidence, Mapping) or set(evidence) != set(POST_QUALIFICATION_EVIDENCE_V3):
        raise RuntimeError("STOP_V5_POSTQUALIFICATION_V3_EVIDENCE_SET_MISMATCH")
    ...
    dependency = _validate_dependency_row_v2(
    ...
    payload = {
        "schema": "JEPA_V5_POSTQUALIFICATION_BUNDLE_V3",
        ...
        "dependency_closure_v2_required": True,
        "executable_power_calibration_required": True,
        "legacy_v3_power_report_disallowed": True,
...
def validate_postqualification_bundle_v3(
    ...
    *,
    ...
    if not isinstance(bundle, Mapping) or bundle.get("schema") != "JEPA_V5_POSTQUALIFICATION_BUNDLE_V3":
        raise ValueError("unexpected postqualification V3 bundle schema")
    if bundle.get("dependency_closure_v2_required") is not True:
        raise RuntimeError("STOP_V5_DEPENDENCY_CLOSURE_V2_NOT_REQUIRED")
    if bundle.get("executable_power_calibration_required") is not True:
        raise RuntimeError("STOP_V5_EXECUTABLE_POWER_CALIBRATION_NOT_REQUIRED")
    if bundle.get("legacy_v3_power_report_disallowed") is not True:
        raise RuntimeError("STOP_V5_LEGACY_V3_POWER_REPORT_NOT_DISALLOWED")
    if bundle.get("production_training_eligible") is not True:
        raise RuntimeError("STOP_V5_POSTQUALIFICATION_V3_NOT_ELIGIBLE")
    if bundle.get("production_training_authorized") is not False:
        raise RuntimeError("STOP_V5_POSTQUALIFICATION_V3_CLAIMS_PRODUCTION_AUTHORITY")

    rebuilt = build_postqualification_bundle_v3(
    ...
    if rebuilt["bundle_sha256"] != bundle.get("bundle_sha256"):
        raise RuntimeError("STOP_V5_POSTQUALIFICATION_V3_BUNDLE_DIGEST_MISMATCH")
    return rebuilt
