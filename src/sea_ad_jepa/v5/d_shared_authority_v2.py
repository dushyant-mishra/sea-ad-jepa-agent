...
def validate_d_shared_authority_v2(authority: Mapping[str, object]) -> dict[str, object]:
    ...
    for row in quantities:
        if not isinstance(row, Mapping):
            raise DSharedAuthorityStop("STOP_D_SHARED_AUTHORITY_QUANTITY_NOT_MAPPING")
        qid = row.get("quantity_id")
        if qid not in _EXPECTED_QIDS or qid in seen:
            raise DSharedAuthorityStop("STOP_D_SHARED_AUTHORITY_QUANTITY_ID")
        seen.add(str(qid))
        if (row.get("rank_min"), row.get("rank_max")) != (1, 512):
            raise DSharedAuthorityStop(f"STOP_D_SHARED_AUTHORITY_RANK_ENVELOPE:{qid}")
        ...
        if any(isinstance(x, bool) or not isinstance(x, (int, float)) for x in (lo, hi, eps, row_alpha)):
            raise DSharedAuthorityStop(f"STOP_D_SHARED_AUTHORITY_QUANTITY_NUMERIC:{qid}")
        if not math.isclose(float(row_alpha), _EXPECTED_ALPHA, rel_tol=0.0, abs_tol=1e-15):
            raise DSharedAuthorityStop(f"STOP_D_SHARED_AUTHORITY_RISK_ALLOCATION:{qid}")
        expected = _hoeffding_n(float(hi) - float(lo), float(eps), float(row_alpha))
        if row.get("replicates_per_rank") != expected:
            raise DSharedAuthorityStop(f"STOP_D_SHARED_AUTHORITY_REPLICATE_COUNT:{qid}")
        plan[str(qid)] = {
            "replicates_per_rank": expected,
            "rank_min": 1,
            "rank_max": 512,
            "lower_bound": float(lo),
            "upper_bound": float(hi),
            "absolute_precision_tolerance": float(eps),
            "risk_allocation_per_rank": float(row_alpha),
        }
    if seen != _EXPECTED_QIDS:
        raise DSharedAuthorityStop("STOP_D_SHARED_AUTHORITY_QUANTITY_SET")
    return {
        "passed": True,
        "terminal": "PASS_V5_PROSPECTIVE_D_SHARED_AUTHORITY_V2",
        "rank_count": 512,
        "alpha_per_quantity_rank": _EXPECTED_ALPHA,
        "matched_null_generator": _EXPECTED_GENERATOR,
        "matching_tuple": list(_EXPECTED_MATCHING),
        "effect_criterion": _EXPECTED_EFFECT,
        "quantity_plan": plan,
...
def bind_d_shared_execution_plan_v2(authority: Mapping[str, object], *, authority_sha256: str) -> dict[str, object]:
    if authority_sha256 != FROZEN_D_SHARED_AUTHORITY_V2_SHA256:
        raise DSharedAuthorityStop("STOP_D_SHARED_EXECUTION_PLAN_UNFROZEN_AUTHORITY_SHA")
    actual = hashlib.sha256(canonical_d_shared_authority_bytes(authority)).hexdigest()
    if actual != authority_sha256:
        raise DSharedAuthorityStop("STOP_D_SHARED_EXECUTION_PLAN_AUTHORITY_BYTES_MISMATCH")
    validated = validate_d_shared_authority_v2(authority)
    return {
        "schema": "JEPA_V5_D_SHARED_PRECISION_EXECUTION_PLAN_V2",
        "d_shared_authority_v2_sha256": authority_sha256,
        "authority_terminal": validated["terminal"],
        "rank_min": 1,
        "rank_max": 512,
        "rank_count": 512,
        "quantity_plan": validated["quantity_plan"],
        "matched_null_generator": validated["matched_null_generator"],
        "matching_tuple": validated["matching_tuple"],
        "effect_criterion": validated["effect_criterion"],
        ...
        "informational_max_replicates_per_rank": max(int(v["replicates_per_rank"]) for v in validated["quantity_plan"].values()),
