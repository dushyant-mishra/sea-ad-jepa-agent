"""Outcome-blind scientific-resolution record for Audit B V2.

V1 unintentionally pre-locked the future weighting choice to the source-balanced
candidate even though the September-21 handoff correctly leaves weighting as a
prospective scientific decision. V2 fixes only that provenance/design mismatch.

It records one of the two currently implemented weighting candidates:
- donor-uniform across all expected donors (the current base scientific mass);
- equal donors within source, then equal sources (a distinct source-balanced estimand).

It does not choose between them, does not authorize N1, and does not authorize
terminal masking or training. Any executable successor contract must bind the
selected weighting explicitly after outcome-blind scientific review.
"""
...
class AuditBScientificResolutionV2:
    resolution_id: str
    ...
    def validate(self) -> None:
        ...
        if self.target_aggregation_id not in ALLOWED_TARGET_AGGREGATION_IDS:
            raise ValueError(
                "target_aggregation_id must name a prospectively implemented candidate"
            )
    ...
    def canonical_digest(self) -> str:
        ...
        return hashlib.sha256(
            _canonical(
                {
                    "schema": "V5_AUDIT_B_SCIENTIFIC_RESOLUTION_V2",
                    **asdict(self),
                }
            )
        ).hexdigest()
