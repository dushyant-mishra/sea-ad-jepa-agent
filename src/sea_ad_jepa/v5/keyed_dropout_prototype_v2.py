"""V4-parameter-compatible encoder using exact V5 Philox reference dropout.

This is deliberately an inactive mechanics reference.  It proves that the V2
injective random-address contract can preserve the retained-token stochastic
objective under dense versus packed execution.  The scalar CPU Philox mask
builder is not a production kernel.
"""
...
from .keyed_rng_contract_v2 import keyed_feature_dropout_reference


class KeyedTokenPreservingBlockV2(nn.Module):
    ...
    def forward(self,tokens:torch.Tensor,valid_mask:torch.Tensor,*,cell_keys:torch.Tensor,token_keys:torch.Tensor,run_seed:int,update_index:int,view_index:int,layer_index:int)->tuple[torch.Tensor,torch.Tensor]:
        ...
        attended=keyed_feature_dropout_reference(
        ...
        hidden=keyed_feature_dropout_reference(
...
class KeyedIPBEncoderV2Reference(nn.Module):
    def __init__(self,*,width:int=160,heads:int=4,blocks:int=6,ffn_width:int=320,dropout:float=.10,vocabulary_size:int=41_238)->None:
        ...
        self.blocks=nn.ModuleList([KeyedTokenPreservingBlockV2(width,heads,ffn_width,dropout) for _ in range(blocks)])
